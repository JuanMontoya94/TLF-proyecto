...
public class AnalizadorLexico {
	...
	public Token extraerOperadorAsignacion(String cod, int i) {
		...
		} else if (cod.charAt(i) == '±' || cod.charAt(i) == '┐' || cod.charAt(i) == '#' || cod.charAt(i) == '÷'
				|| cod.charAt(i) == 'R') {
			j = i + 1;
			if (j < cod.length()) {
				if (cod.charAt(j) == '═') {
	...
	// cod.charAt(i)=='±'||cod.charAt(i)=='┐'||cod.charAt(i)=='#'
	...
	public Token extraerOperadorAritmetico(String cod, int i) {
		...
		if (cod.charAt(i) == '±') {
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICOS, j);
			return token;
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICOR, j);
			return token;
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICOM, j);
			return token;
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICOD, j);
			return token;
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICORE, j);
			return token;
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICOPO, j);
			return token;
			...
			lex = cod.substring(i, j);
			Token token = new Token(lex, Token.OPERADORARITMETICORA, j);
			return token;
